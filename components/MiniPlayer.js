...
const MiniPlayer = ({ clip }) => {
  ...
  const handleSingle = async () => {
      try {
        const result = await axios.post(`${baseUrl}api/getClip`, {id: clip._id})
    ...
    return (
      <div className=" flex-col items-start justify-start my-1 mx-1 ">
        ...
        <div className="flex flex-row justify-between font-light">
          ...
          <Link onClick={handleSingle} className='text-white font-semibold opacity-80 vidDets mr-1 hover:animate-pulse' href={`/`}>Details</Link>
        </div>
      </div>
    );
}
